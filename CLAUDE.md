...
- **Role assignment is a two-tier privilege, not president-only.** `ROLE_ADMIN_ROLES = {president, vpe, vpi}` (`models/user/user_enums.py`, next to `SHOP_ADMIN_ROLES`) gates all four `/admin/*` endpoints via `require_role_admin` in `services/dependencies.py` (mirrors `require_shop_admin`; it replaced the old `require_president`, which no longer exists). The **VP-specific limits are per-request, not in the gate** — `_assert_may_assign` in `admin_routes.py` 403s a non-president who tries to (a) assign `Role.president` or (b) modify a target who currently *is* president. Both are needed: without (b) any VP could demote the president and leave the chapter with no full admin. `GET /admin/roles` also filters `President` out of a VP's list so the dropdown can't offer a pick the PATCH would reject — that's convenience; `_assert_may_assign` is the enforcement, keep both.
...
- Frontend: `utils/shop.js` has `isPresident()` + `PRESIDENT_ROLE`, `canAssignRoles()` + `ROLE_ADMIN_ROLES` (display-value strings mirroring the backend set), and its `SHOP_ADMIN_ROLES` list includes `"President"`. `pages/members.jsx` (route `/members`) guards with `canAssignRoles` and fetches members/stats/roles once, filtering client-side (`api/api.js`: `getAdminMembers`, `getAdminStats`, `getAssignableRoles`, `updateMemberRole`); `Header.jsx`'s `memberLinksFor` inserts the Members tab (president + both VPs) and Shop Manager tab (any shop admin) after Committees; the profile hero badge shows "★ President" instead of "★ Shop Manager" for the president.
...
- **Controlled `<select>` + `window.confirm` gotcha** (members.jsx role dropdown): if the confirm is cancelled, React state never changes so it won't repaint, but the DOM select already shows the new option — reset `e.target.value` manually in the onChange handler. The message itself comes from `roleChangeWarning()`, which names both the old and new role and appends a committee-listing warning when either side is a chair role (detected by the display value ending in `"Chair"` — true for all 14).
...
- Tests in `tests/admin_tests/` — `president_client` and `vp_client` fixtures, same "don't mix two auth'd clients in one test" rule as `manager_client`.

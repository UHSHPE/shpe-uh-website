...
- `seed.py`'s `DEFAULT_REPORTS` is a **plausible starting tree, not the chapter's real one** (nobody specified who actually oversees whom). It covers all 6 officers and all 14 chairs so the tab isn't empty on first load, and the president/VPs are expected to correct it under Members → Structure.

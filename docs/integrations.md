...
A verified claim sets the member's stored `has_paid_dues` flag, the same flag a website purchase sets — an import creates no shop order. Claims for any period other than the current one are recorded and grant nothing, so starting next year's tab early pays no one. Payments are recognized even when the member registers after the import: `create_user` matches a waiting claim by PSID at signup. Dues are **final once paid** — a dues order cannot be cancelled — and are retired by `reset_dues.py` every May 30. Repeated imports and duplicate PSIDs create no duplicate claims; a checked duplicate wins. Once verified, a claim remains verified if its checkbox is later unchecked or its row disappears. Payment reversals require a deliberate correction to the imported record; sheet edits alone do not revoke dues.
